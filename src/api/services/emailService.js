...
// Escape values interpolated into HTML email bodies. Component names/versions
// come from parsed lock files and vuln titles come from OSV (third-party data),
// so they must not be trusted as literal HTML.
function esc(s) {
    return String(s ?? '')
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
...
async function sendVulnAlertIfNew(orgId, appId, appName) {
    try {
        ...
        const tableRows = newCrits.map(v =>
            `<tr><td style="padding:6px 12px;border-bottom:1px solid #30363d">${esc(v.cve_id || v.osv_id)}</td>` +
            `<td style="padding:6px 12px;border-bottom:1px solid #30363d">${esc(v.component)} ${esc(v.version)}</td>` +
            `<td style="padding:6px 12px;border-bottom:1px solid #30363d">${esc(v.cvss_score ?? '—')}</td>` +
            `<td style="padding:6px 12px;border-bottom:1px solid #30363d;color:#8b949e;font-size:12px">${v.title ? esc(v.title.slice(0, 80)) : '—'}</td></tr>`
        ).join('');
...
<p style="color:#8b949e;margin-bottom:24px"><strong style="color:#e6edf3">${esc(appName)}</strong> has ${newCrits.length} new critical finding${newCrits.length > 1 ? 's' : ''} since its last scan.</p>
    ...
    try {
        ...
        await sendEmail({
            ...
            html: '<!DOCTYPE html><html><body style="background:#0d1117;color:#e6edf3;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;padding:40px;max-width:600px;margin:0 auto">'
                ...
                + '<h1 style="font-size:22px;font-weight:700;margin:0 0 20px">' + esc(appName) + '</h1>'
